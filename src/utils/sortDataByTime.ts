...
export const sortDataByTime = ({ reposts, posts }: PostsArrayType) => {
    ...
    const combinedPostId = combinedPosts.map((post)=> post.type === 'post' ? {id: post.id, created_at: post.created_at} : {id: post.post_id, created_at: post.reposted_at}).slice(0,10);
